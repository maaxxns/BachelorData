...
out=[]
line1=[]
Comment='Without Preamplifier,Fluence 1 filter = NE15A-B, ZnTe detector angle dependance 40deg, Chopper 230Hz, time constant='+str(time_constant[time_con])+'s '+str(dB_values[dB])+'dB, waiting time= '+str(np.round(waiting_time, 4))+'s, sensitivity='+set_sens # Comment for meta data
pos=0
...
#%% 
plt.close('all')
position = 11.01  #set position of test measurment 
length = 1000
out=[]
line1=[]
...
for i in x:
    ...
    print(plot_y[pos])
    #line1=live_plotter(x,np.array(X),line1)
    line1=live_plotter(x_axis,plot_y,line1,start=x_axis[0],stop=x_axis[-1])
    plt.ylim(np.min(plot_y[0:pos+1])-np.std(plot_y[0:pos+1]),np.max(plot_y[0:pos+1])+np.std(plot_y[0:pos+1]))
    pos+=1
