# -*- coding: utf-8 -*-
"""
Created on Thu Jun  2 10:49:37 2022
...
#				  its coreesponding index inside of the SR830 serial.
	...
	def __init__(self,resourceLoc='ASRL4::INSTR'):#"GPIB0::5::INSTR"):
		...
		self.time_constantdB = {"6dB":"0","12dB":"1","18dB":"2","24dB":"3"}
		self.dB = [6,12,18,24]
		#self.setIT()
		#self.setSens()
		self.setSync()
		...
		self.idB = self.dB[int(self.inst.query("OFSL ?")[:-1])]
	...
	def setdB(self,name=None, i=0):
		"""
        dB Values are as follows:
        {dB/oct (i=0), 12 dB/oct (i=1), 18 dB/oct (i=2),
        24 dB/oct (i=3)}
       """ 
		waiting_constant = [5, 7, 9, 10]
		print('Waiting time for '+str(self.dB[i])+'dB should be atleast ' +str(waiting_constant[i])+'times the time constant')
		if name!=None:
			self.inst.write("OFSL "+self.time_constantdB[name])
			time.sleep(0.1)
			self.idB = self.dB[int(self.inst.query("OFSL ?")[:-1])]
		else:
			self.inst.write("OFSL "+str(int(i)))
			time.sleep(0.1)
			self.idB = self.dB[i]
	def getSens(self):
	...
	def getOut(self,i=3):
		...
		"""
		assert i<5 and i>0
		return(float(self.inst.query("OUTP? "+str(i)).rstrip(':')))
	def getRTh(self):
		return(np.array([float(self.inst.query("OUTP? "+str(3)).rstrip(':')),float(self.inst.query("OUTP? "+str(4)).rstrip(':'))]))		
	def get_All_Outputs(self):
		return(np.array([float(self.inst.query("OUTP? "+str(1)).rstrip(':')),float(self.inst.query("OUTP? "+str(2)).rstrip(':')),float(self.inst.query("OUTP? "+str(3)).rstrip(':')),float(self.inst.query("OUTP? "+str(4)).rstrip(':'))]))		
#	def get_All_Outputs_2systems(self): #use this function if two Lock in are used. It strips the string of the ':' in the output, which seems to mess with the output
#		return(np.array([float(self.inst.query("OUTP? "+str(1)).rstrip(':')),float(self.inst.query("OUTP? "+str(2)).rstrip(':')),float(self.inst.query("OUTP? "+str(3)).rstrip(':')),float(self.inst.query("OUTP? "+str(4)).rstrip(':'))]))		
	def getXY(self):
		return(np.array([float(self.inst.query("OUTP? "+str(1)).rstrip(':')),float(self.inst.query("OUTP? "+str(2)).rstrip(':'))]))
